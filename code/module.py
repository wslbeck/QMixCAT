...
#         16,   16,   16,   16,   16,  16,  16,   17])
#         for i in range(len(self.band)):
#             setattr(self, 'norm{}'.format(i + 1), nn.GroupNorm(1, 2))
#             setattr(self, 'fc{}'.format(i + 1), nn.Linear(int(self.band[i]), int(self.band[i])))
...
#             # x_band = torch.reshape(x_band,[x_band.size(0),x_band.size(1),x_band.size(2)*x_band.size(3)])
#             out = getattr(self, 'norm{}'.format(i + 1))(x_band)
#             out = getattr(self, 'fc{}'.format(i + 1))(out)

#             if i == 0:
#                 m = out
#             else:
#                 m = torch.cat((m, out), dim=3)
#         return m.permute(0, 3, 2, 1)
